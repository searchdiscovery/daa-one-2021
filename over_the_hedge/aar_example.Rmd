...
## setup
source('aasource.R', keep.source = T)
...
df <- aw_freeform_table(date_range = dr,
...
1.  *date* - dimension - This is the date in the 'YYYY-mm-dd' format
2.  *visits* - metric - `r mets$description[mets$id =='visits']` *(this would show a description if available but since the description field is blank you would fill in the definition)*

## Visualize the Data

```{r vizualize data, warning=FALSE, echo=FALSE, message=FALSE}
df %>%
  mutate(date = as.Date(daterangeday)) %>%
  mutate(`Weekday` = wday(date, label = T)) %>%
  ggplot(aes(`Weekday`, visits, fill = month(date, label = T))) + 
  geom_col(position = 'dodge') +
  theme_classic() +
  theme(legend.title = element_blank()) +
  labs(title = 'Visits by Weekday and Month, Yes!')
```
...
I'm recommending we run more ads on the first 3 days of the week because thos days have a majority of traffic and if our assumption is right we will see an increase in response more than on days we have less traffic.
